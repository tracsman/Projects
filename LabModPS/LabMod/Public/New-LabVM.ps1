    ...
    If ($PostBuildOnly) {
        Switch ($OS) {
            ...
            "Ubuntu" {
                ...
                $script += "sed -i '/routes:/a\ \ \ \ \ \ - to: ::\/0\n\ \ \ \ \ \ \ \ via: $VMGWv6' /etc/netplan/50-cloud-init.yaml`n"
                #$script += "sed -i '/eth0:/,/addresses:/ s/addresses:/addresses:\n\ \ \ \ \ \ - $VMIPv6\/64' /etc/netplan/50-cloud-init.yaml`n"
                $script += "sed -i '/eth0:/,/^[[:space:]]\{6\}addresses:/ s/^[[:space:]]\{6\}addresses:/      addresses:\n      - $VMIPv6\/64/' /etc/netplan/50-cloud-init.yaml`n"
                $script += "sed -i '/nameservers:/,/addresses:/ s/addresses:/addresses:\n\ \ \ \ \ \ \ \ - 2606:4700:4700::1111\n\ \ \ \ \ \ \ \ - 2606:4700:4700::1001/' /etc/netplan/50-cloud-init.yaml`n"
